...
{
    objs.clear();
    auto input_h      = this->input_bindings[0].dims.d[2]; //입력 및 출력 바인딩에서 필요한 정보를 가져옵니다. 예를 들어, 입력 이미지의 높이와 너비, 앵커의 수, 출력 채널의 수 등을 가져옵니다.
    auto input_w      = this->input_bindings[0].dims.d[3];
    auto num_anchors  = this->output_bindings[0].dims.d[1]; // 앵커는 객체 검출 네트워크에서 사용되는 사전 정의된 박스의 크기와 종횡비를 나타냅니다.
    auto num_channels = this->output_bindings[0].dims.d[2];
    // 객체 후처리에 필요한 파라미터들을 가져옵니다. 이는 확률 맵과 박스 크기에 대한 정보입니다.
    auto& dw     = this->pparam.dw;
    auto& dh     = this->pparam.dh;
    auto& width  = this->pparam.width;
    auto& height = this->pparam.height;
    auto& ratio  = this->pparam.ratio;
    // 출력 데이터를 가리키는 포인터를 output에 할당
    auto*   output = static_cast<float*>(this->host_ptrs[0]);
    // 세그멘테이션 맵을 가리키는 포인터를 protos에 할당합니다.
    cv::Mat protos = cv::Mat(seg_channels, seg_h * seg_w, CV_32F, static_cast<float*>(this->host_ptrs[1]));
    // 객체 검출 결과를 저장할 벡터들을 초기화합니다.
    std::vector<int>      labels;
    std::vector<float>    scores;
    std::vector<cv::Rect> bboxes;
    std::vector<cv::Mat>  mask_confs;
    std::vector<int>      indices;
    // 각 앵커에 대해 반복하면서, 확률이 임계값(score_thres)을 초과하는 객체를 필터링하고, 박스의 좌표 및 크기를 조정합니다.
...
// cv::dnn::NMSBoxes 또는 cv::dnn::NMSBoxesBatched를 사용하여 비최대 억제를 수행합니다. 이는 겹치는 박스를 제거하고, 상위 topk개의 박스를 선택합니다.
#if defined(BATCHED_NMS)
    cv::dnn::NMSBoxesBatched(bboxes, scores, labels, score_thres, iou_thres, indices);
#else
    cv::dnn::NMSBoxes(bboxes, scores, score_thres, iou_thres, indices);
#endif
    // 선택된 객체의 정보를 objs 벡터에 저장하고, 세그멘테이션 맵을 활용하여 객체의 마스크를 추출합니다.
    cv::Mat masks;
    int     cnt = 0;
    for (auto& i : indices) {
        if (cnt >= topk) {
                              ...
                              const std::vector<std::vector<unsigned int>>& MASK_COLORS)
{
    ...
    for (auto& obj : objs) {
        ...
        // 텍스트가 이미지를 벗어날 경우 위치 조정
        if (y > res.rows)
            y = res.rows;
        // 객체 정보를 표시하는 빨간색 상자 그림 ==> class id and prob imshow
        cv::rectangle(res, cv::Rect(x, y, label_size.width, label_size.height + baseLine), {0, 0, 255}, -1);

        cv::putText(res, text, cv::Point(x, y + label_size.height), cv::FONT_HERSHEY_SIMPLEX, 0.4, {255, 255, 255}, 1);
    }
    // 이미지와 마스크를 결합하여 시각적인 가중치 부여 어떠한 이미지를 얼마나 반영할지 결정
